...
export const LeaveDeductions = () => {
  ...
  const [entries, setEntries] = useState<LeaveDeductionEntry[]>([]);
  const [loading, setLoading] = useState(false);
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const [showEditDialog, setShowEditDialog] = useState(false);
  const [editingEntry, setEditingEntry] = useState<LeaveDeductionEntry | null>(null);
  ...
  const enrichEntry = useCallback((row: any): LeaveDeductionEntry => {
  ...
  const fetchEntries = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from('leave_deductions').select('*').order('created_at', { ascending: false });
      ...
      try {
        ...
        if (payload.length === 0) {
          toast({
            ...
            description: isRTL ? 'لم يتم العثور على بيانات صالحة. تأكد أن الملف يحتوي على عمودين: رقم ID الموظف وعدد أيام الإجازة' : 'No valid data found. Ensure file has two columns: Employee ID and Leave Days',
            variant: 'destructive',
          });
        } else {
          const { error } = await supabase.from('leave_deductions').upsert(payload, { onConflict: 'employee_id,deduction_month' });
          if (error) {
  ...
  const handleDelete = async () => {
    ...
    const { error } = await supabase.from('leave_deductions').delete().eq('id', deletingId);
    if (!error) {
  ...
  const handleMarkDeducted = async (id: string) => {
    const { error } = await supabase.from('leave_deductions').update({ status: 'deducted' }).eq('id', id);
    if (!error) {
  ...
  const handleApplyAllForMonth = async () => {
    ...
    const { error } = await supabase.from('leave_deductions').update({ status: 'deducted' }).in('id', ids);
    if (error) {
      toast({ title: isRTL ? 'خطأ' : 'Error', description: error.message, variant: 'destructive' });
      return;
    }
  ...
  const handleBulkDeduct = async () => {
    ...
    const { error } = await supabase.from('leave_deductions').update({ status: 'deducted' }).in('id', pendingIds);
    if (!error) {
  ...
  const handleEditSave = async () => {
    ...
    const { error } = await supabase.from('leave_deductions').update({ days: newDays }).eq('id', editingEntry.id);
    if (!error) {
  ...
  const exportTitle = isRTL ? 'تقرير خصم الإجازات' : 'Leave Deductions Report';
  const exportColumns = [
    ...
    { header: isRTL ? 'عدد أيام الإجازة' : 'Leave Days', key: 'days' },
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className={cn("flex items-center gap-2", isRTL && "flex-row-reverse")}>
            ...
            {isRTL ? 'رفع أيام الإجازات' : 'Upload Leave Days'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className={cn("flex flex-col md:flex-row gap-4 items-end", isRTL && "md:flex-row-reverse")}>
            ...
            <div className="space-y-2 flex-1">
              <Label>{isRTL ? 'ملف الإجازات (Excel/CSV) *' : 'Leaves File (Excel/CSV) *'}</Label>
          ...
          <div className={cn("mt-4 p-4 rounded-lg bg-muted/50 border border-border/50", isRTL && "text-right")}>
            ...
            <ul className={cn("text-xs text-muted-foreground space-y-1", isRTL ? "list-disc pr-5" : "list-disc pl-5")}>
              ...
              <li>{isRTL ? 'العمود الثاني: عدد أيام الإجازة المخصومة من الراتب' : 'Column 2: Number of leave days deducted from salary'}</li>
      ...
      <Card>
        <CardHeader>
          <div className={cn("flex flex-col md:flex-row md:items-center md:justify-between gap-4", isRTL && "md:flex-row-reverse")}>
            <CardTitle>{isRTL ? 'سجل خصومات الإجازات' : 'Leave Deductions Log'}</CardTitle>
            <div className={cn("flex flex-wrap gap-3", isRTL && "flex-row-reverse")}>
              ...
              <Button variant="outline" size="icon" onClick={() => exportToCSV({ title: exportTitle, data: exportData, columns: exportColumns, fileName: 'leave-deductions' })}><FileSpreadsheet className="h-4 w-4" /></Button>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className={cn(isRTL && "text-right")}>{isRTL ? 'عدد أيام الإجازة' : 'Leave Days'}</TableHead>
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{isRTL ? 'تعديل عدد الأيام' : 'Edit Leave Days'}</DialogTitle>
          </DialogHeader>
          {editingEntry && (
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                <Label>{isRTL ? 'عدد أيام الإجازة' : 'Leave Days'}</Label>
