  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-6">
          {/* Previous quarters' bonus percentages for the selected employee */}
          {selectedEmployee && (() => {
            const empReviews = reviews
              .filter(r => r.employeeId === selectedEmployee && r.bonusPercentage != null)
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className={cn("text-base flex items-center gap-2", isRTL && "flex-row-reverse")}>
                    ...
                    {ar ? 'نسب مكافآت الأرباع السابقة' : 'Previous Quarters Bonus %'}
                    <Badge variant="secondary" className="ms-2">{ar ? 'متوسط عام' : 'Overall Avg'}: {overallAvg}%</Badge>
                  </CardTitle>
                </CardHeader>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {perQuarter.map(p => (
                      <div key={p.q} className="rounded-lg border border-border bg-muted/30 p-3 text-center">
                        ...
                        <div className="text-[10px] text-muted-foreground mt-1">{p.count} {ar ? 'تقييم' : 'review(s)'}</div>
                      </div>
                    ))}
                  </div>
                  {empReviews.length > 0 ? (
                    <div className="overflow-x-auto">
                      <table className="w-full text-xs">
                        <thead className="bg-muted/40">
                          <tr>
                            <th className="text-start p-2">{ar ? 'الربع' : 'Quarter'}</th>
                            <th className="text-center p-2">{ar ? 'السنة' : 'Year'}</th>
                            <th className="text-center p-2">{ar ? 'الدرجة' : 'Score'}</th>
                            <th className="text-center p-2">{ar ? 'المكافأة' : 'Bonus'}</th>
                          </tr>
                        </thead>
                        <tbody>
                          {empReviews.slice(-8).map(r => (
                            <tr key={r.id} className="border-t border-border/40">
                              ...
                              <td className="p-2 text-center"><span className="inline-block px-2 py-0.5 rounded-full bg-stat-green/15 text-stat-green font-bold">{r.bonusPercentage}%</span></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="text-center text-xs text-muted-foreground py-2">{ar ? 'لا توجد بيانات مكافآت سابقة لهذا الموظف' : 'No prior bonus data for this employee'}</div>
                  )}
                </CardContent>
              </Card>
            );
