...
export async function exportMonthlyByStationExcel(input: MBSInput): Promise<void> {
  ...
  const headers = ar
    ? ['المحطة','الشهر','العدد','الأساسي','مواصلات','حوافز','بدل محطة','بدل محمول','بدل معيشة','أجر إضافي','مكافآت','الإجمالي','تأمينات','قروض','سلف','فاتورة محمول','خصم إجازات','جزاءات','إجمالي خصومات','الصافي','تأمينات ص.ع','صحي','ضريبة','إجمالي مساهمات ص.ع']
    : ['Station','Month','Count','Basic','Trans.','Incent.','St.All.','Mob.','Living','OT','Bonus','Gross','Ins.','Loans','Adv.','Mob.Bill','Leave Ded.','Penalty','Tot.Ded','Net','Emp.Ins','Health','Tax','Total Employer'];
  ...
  const emptyTotals = () => ({ count: 0, basic: 0, transport: 0, incentives: 0, stationAllowance: 0, mobileAllowance: 0, livingAllowance: 0, overtimePay: 0, bonuses: 0, gross: 0, insurance: 0, loans: 0, advances: 0, mobileBill: 0, leaveDeduction: 0, penalty: 0, totalDeductions: 0, net: 0, employerInsurance: 0, healthInsurance: 0, incomeTax: 0 });
  ...
  const renderTotalRow = (label: string, totals: ReturnType<typeof emptyTotals>, isGrand: boolean) => {
    ...
    const vals = [totals.count, totals.basic, totals.transport, totals.incentives, totals.stationAllowance, totals.mobileAllowance, totals.livingAllowance, totals.overtimePay, totals.bonuses, totals.gross, totals.insurance, totals.loans, totals.advances, totals.mobileBill, totals.leaveDeduction, totals.penalty, totals.totalDeductions, totals.net, totals.employerInsurance, totals.healthInsurance, totals.incomeTax, totals.employerInsurance + totals.healthInsurance + totals.incomeTax];
    ...
    for (let c = 1; c <= colCount; c++) {
      ...
      else if (c === 19) cell.font = { ...cell.font, color: { argb: C.destructive } };
      else if (c === 20) fill(cell, isGrand ? C.grandNetBg : C.netSubBg);
      else if (c === 24) cell.font = { ...cell.font, color: { argb: C.blueBold } };
    }
