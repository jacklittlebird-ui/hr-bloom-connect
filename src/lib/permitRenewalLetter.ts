...
const AIRPORTS_RENEWAL_TEMPLATE_URL = '/templates/security-airports-renewal.docx';

/** خطاب تجديد تصاريح المطارات (قطاع الأمن) */
export async function exportSecurityAirportsRenewalLetter(
  rows: LetterRow[],
  year: string,
  dateText: string,
  purpose = 'إنهاء إجراءات الركاب',
  airports = 'عموم المطارات',
  fileName = `خطاب_تجديد_تصاريح_المطارات_${year}.docx`,
) {
  const res = await fetch(AIRPORTS_RENEWAL_TEMPLATE_URL);
  if (!res.ok) throw new Error('template not found');
  const zip = await JSZip.loadAsync(await res.arrayBuffer());
  let xml = await zip.file('word/document.xml')!.async('string');
