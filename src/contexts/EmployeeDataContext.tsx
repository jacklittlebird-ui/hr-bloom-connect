...
function mapRow(row: any): Employee {
  return {
    ...
    socialInsuranceClosedDate: row.social_insurance_closed_date || undefined,
    documentsOriginalsReceived: row.documents_originals_received ?? undefined,
...
async function mapUpdates(updates: Partial<Employee>): Promise<Record<string, any>> {
  const map: Record<string, string> = {
    ...
    socialInsuranceClosedDate: 'social_insurance_closed_date',
    documentsOriginalsReceived: 'documents_originals_received',
      ...
      const result = await debouncedFetch(cacheKey, async () => {
        ...
        if (user?.role === 'training_manager') {
          const [empRes, deptsRes, stationsRes] = await Promise.all([
            fetchAllPaginated(() => supabase.from('employees').select(TRAINING_MANAGER_LIST_SELECT).in('status', ['active', 'pending_hire', 'external_stations']).order('employee_code', { ascending: true })),
            supabase.from('departments').select('id, name_ar, name_en'),
            supabase.from('stations').select('id, code, name_ar, name_en'),
          ]);
