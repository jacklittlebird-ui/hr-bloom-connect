...
<div class="WordSection1">
  ...
  const buildPdfHtml = useCallback(async ({ title, data, columns }: ReportExportOptions): Promise<string> => {
    const tableRows = data.map(row =>
      `<tr>${columns.map(col => `<td style="border:1px solid #d1d5db;padding:10px 12px;font-size:12px;text-align:${isRTL ? 'right' : 'left'};color:#1f2937;">${escapeHtml(row[col.key])}</td>`).join('')}</tr>`
    ).join('');

    const logoData = await loadImageAsDataUrl(logoUrl);
    const logoTag = logoData ? `<img src="${logoData}" style="height:60px;width:auto;" />` : '';
  ...
  const previewPDF = useCallback(async (opts: ReportExportOptions): Promise<string | null> => {
    if (!opts.data.length) {
      toast({ title: t('reports.noData') || 'No data to export', variant: 'destructive' });
      return null;
    }
  ...
  const buildBilingualPdfHtml = useCallback(async ({ titleAr, titleEn, data, columns, summaryCards }: BilingualExportOptions): Promise<string> => {
    const dir = isRTL ? 'rtl' : 'ltr';
    const tableRows = data.map(row =>
  ...
  const previewBilingualPDF = useCallback(async (opts: BilingualExportOptions): Promise<string | null> => {
    if (!opts.data.length) {
      toast({ title: t('reports.noData') || 'No data to export', variant: 'destructive' });
      return null;
    }
