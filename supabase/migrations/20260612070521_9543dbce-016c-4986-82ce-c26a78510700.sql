...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    PERFORM notify_employee_and_admins(
      ...
      CASE WHEN NEW.status = 'draft' THEN NULL ELSE 'الدرجة: ' || COALESCE(NEW.score::text, 'لم تحدد بعد') END,
      CASE WHEN NEW.status = 'draft' THEN NULL ELSE 'Score: ' || COALESCE(NEW.score::text, 'Not set yet') END,
      'info', 'performance'
    );
