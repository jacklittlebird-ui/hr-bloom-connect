...
BEGIN
  ...
  gross_before_bonus := COALESCE(NEW.basic_salary, 0)
  ...
  IF NEW.bonus_type = 'percentage' THEN
    NEW.bonus_amount := ROUND(gross_before_bonus * COALESCE(NEW.bonus_value, 0) / 100, 2);
  ELSE
    NEW.bonus_amount := COALESCE(NEW.bonus_value, 0);
  END IF;

  NEW.gross := gross_before_bonus + COALESCE(NEW.bonus_amount, 0);
...
WHERE month = '07' AND year = '2026' AND bonus_type = 'percentage' AND COALESCE(bonus_value,0) <> 0;